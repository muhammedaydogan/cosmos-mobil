...
export const OtherProfile = () => {
  ...
  const getFeed =() => {
    axios.post('http://192.168.1.125:9000'+"/user",{
      ...
      console.log("0xCAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
      setMe(data.data.result[0]);

      console.log("1=CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
      setPhotos(data.data.result[0].posts);
    })
  }
  React.useEffect(()=>{
    getFeed()
    ...
    <>
      <View style={classes.topView}>
        <View style={classes.topLeftSide}>
          {photos ? (
            <Image
              ...
              source={{
                uri: "http://192.168.1.125:9000/getPhoto/" + photos[1].id,
              }}
            />
          ) : null}
        </View>
        <View style={classes.topRightSide}>
          ...
          <Text style={classes.userNameText}>
            {me.firstName + " " + me.lastName}
          </Text>
          <Text style={classes.userNameSubText}>
            {"Influencer - Fashion/Lifestyle"}
          </Text>
        </View>
      </View>
